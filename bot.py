...
# Server-specific logging setup
server_loggers = {}

def get_server_logger(guild_id, guild_name):
    if guild_id not in server_loggers:
        safe_name = "".join(c if c.isalnum() or c in ('-', '_') else '_' for c in guild_name)
        log_file = f"{guild_id}_{safe_name}.log"
        
        logger = logging.getLogger(f"server_{guild_id}")
        ...
        server_loggers[guild_id] = logger
    
    return server_loggers[guild_id]
...
ATTACHMENTS_DIR = "./discord_stenographer/attachments"
import os
...
# Voice sample storage for cloning
VOICE_SAMPLES_DIR = "voice_samples"
os.makedirs(VOICE_SAMPLES_DIR, exist_ok=True)


async def search_spotify_play_youtube(query, vc, guild_id):
    """Search Spotify for song metadata, then play from YouTube"""
    try:
        if not sp:
            return None
        ...
        youtube_query = f"{artist} {song_name} official audio"
        
        print(f"🎵 Spotify found: {artist} - {song_name}")
        print(f"🔍 Searching YouTube for: {youtube_query}")
        ...
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            search_results = ydl.extract_info(f"ytsearch1:{youtube_query}", download=False)
            
            if search_results and 'entries' in search_results:
                video = search_results['entries'][0]
                video_url = f"https://www.youtube.com/watch?v={video['id']}"
                
                # Now get full info and play
                ydl_opts_full = {
                    'format': 'bestaudio/best',
                    'quiet': True,
                    'no_warnings': True,
                ...
                with yt_dlp.YoutubeDL(ydl_opts_full) as ydl_full:
                    ...
                    # Play it
                    audio_source = discord.FFmpegPCMAudio(audio_url, options='-vn')
                    vc.play(audio_source, after=lambda e: print(f"Finished playing: {song_name}"))
                    
                    return f"Playing: {artist} - {song_name}"
...
async def clone_voice_and_speak(username, text, vc, guild_id):
    """Clone a user's voice and make them say text"""
    try:
        # Find user's voice samples
        safe_username = "".join(c if c.isalnum() or c in ('-', '_') else '_' for c in username)
        sample_dir = f"{VOICE_SAMPLES_DIR}/{safe_username}"
        
        if not os.path.exists(sample_dir):
            return f"No voice samples for {username}"
        
        # Get sample files
        import glob
        samples = glob.glob(f"{sample_dir}/*.wav")
        
        if len(samples) < 10:
            return f"Not enough samples for {username} (need 10+, have {len(samples)})"
        
        print(f"🎭 Cloning {username}'s voice with {len(samples)} samples...")
        
        # Use first 20 samples for speed
        training_samples = samples[:20]
        
        # For now, use gTTS as placeholder (piper voice cloning needs more setup)
        # TODO: Implement actual piper voice cloning
        from gtts import gTTS
        import tempfile
        
        clone_text = f"{username} would say: {text}"
        with tempfile.NamedTemporaryFile(delete=False, suffix='.mp3') as fp:
            tts = gTTS(text=clone_text, lang='en')
            tts.save(fp.name)
            
            audio_source = discord.FFmpegPCMAudio(fp.name)
            vc.play(audio_source, after=lambda e: os.unlink(fp.name))
        
        return f"Playing cloned voice of {username}"
        
    except Exception as e:
        print(f"Voice cloning error: {e}")
        return f"Failed to clone {username}'s voice"


def is_clean_sample(audio_data, transcription):
    """Check if audio sample is clean enough for voice cloning"""
    if len(transcription.split()) < 3:
        return False
    
    if len(audio_data) < 32000:  # Less than 2 seconds at 16kHz
        return False
...
whisper_model = WhisperModel("large-v3-turbo", device=device, compute_type="float16" if device == "cuda" else "int8")
...
music_queues = {}
recent_music_searches = {}  # Track recent band/artist searches

recent_band_searches = {}  # Track what band was just searched
...
MANIPULATION_PATTERNS = {
    'guilt_trip': ["after all i've done", "i guess i'm just a bad", "fine, whatever", "if you really cared"],
    'gaslighting': GASLIGHTING_INDICATORS,
    'deflection': ["what about when you", "yeah but you", "at least i don't"],
    'victimhood': ["everyone always", "nobody ever", "i'm always the one"],
    'emotional_blackmail': ["if you loved me", "a real friend would", "i thought we were"],
}

async def detect_gaslighting(current_statement, username, guild_id):
    """Detect if someone is gaslighting by comparing to past statements"""
    try:
        statement_lower = current_statement.lower()
        
        # Check for gaslighting indicators
        for indicator in GASLIGHTING_INDICATORS:
            if indicator in statement_lower:
                # Search for contradicting past statements
                conn = get_db_connection()
                c = conn.cursor()
                ...
                c.execute("""SELECT transcription, timestamp FROM transcriptions 
                         ...
                         (str(guild_id), username, cutoff))
                past_statements = c.fetchall()
                conn.close()
                
                # Extract key claim from current denial
                if "never said" in statement_lower:
                    # They're denying they said something
                    # Check if we have evidence they DID say it
                    
                    # Simple keyword matching (can be improved)
                    words_in_current = set(current_statement.lower().split())
                    
                    for past_text, past_time in past_statements:
                        past_words = set(past_text.lower().split())
                        
                        # Check for significant overlap
                        overlap = words_in_current & past_words
                        if len(overlap) > 3:  # At least 3 common words
                            # GASLIGHTING DETECTED
                            from datetime import datetime
                            past_dt = datetime.fromisoformat(past_time)
                            time_ago = datetime.now() - past_dt
                            
                            if time_ago.days > 0:
                                when = f"{time_ago.days} days ago"
                            elif time_ago.seconds > 3600:
                                when = f"{time_ago.seconds // 3600} hours ago"
                            else:
                                when = f"{time_ago.seconds // 60} minutes ago"
                            
                            return True, f"Actually, {username} said something similar {when}: '{past_text[:80]}'"
                
                return True, f"{username} is using gaslighting language: '{indicator}'"
        
        return False, None
        
    except Exception as e:
        print(f"Gaslighting detection error: {e}")
        return False, None
...
async def gaslighting_monitor(guild_id):
    """Monitor for gaslighting attempts"""
    await asyncio.sleep(30)
    last_checked_id = 0
    
    while GASLIGHTING_DETECTOR_ENABLED.get(guild_id, False):
        try:
            conn = get_db_connection()
            c = conn.cursor()
            c.execute("""SELECT rowid, username, transcription FROM transcriptions 
            ...
            for row in results:
                ...
                is_gaslighting, evidence = await detect_gaslighting(transcription, username, guild_id)
                
                if is_gaslighting and evidence:
                    print(f"🚨 GASLIGHTING DETECTED: {evidence}")
                    
                    # INTERRUPT
                    if guild_id in voice_clients and 'vc' in voice_clients[guild_id]:
                        vc = voice_clients[guild_id]['vc']
                        await asyncio.to_thread(speak_text, evidence, vc, guild_id)
            
            await asyncio.sleep(2)
            
        except Exception as e:
            print(f"Gaslighting monitor error: {e}")
            await asyncio.sleep(5)
...
async def analyze_shit_talk(target_username, guild_id):
    """Analyze who talks about target when they're not around"""
    try:
        from datetime import datetime, timedelta
        
        # Get who's currently in voice
        current_users = set()
        if guild_id in voice_clients and 'vc' in voice_clients[guild_id]:
            vc = voice_clients[guild_id]['vc']
            current_users = {m.display_name for m in vc.channel.members if not m.bot}
        
        # Find times when target was NOT in voice (last 7 days)
        cutoff = (datetime.now() - timedelta(days=7)).isoformat()
        
        conn = get_db_connection()
        c = conn.cursor()
        
        # Get all mentions of target when they weren't present
        # This is approximate - we assume they weren't there if they weren't transcribed
        c.execute("""SELECT username, transcription, timestamp 
                    FROM transcriptions 
                    WHERE guild_id = ? 
                    AND timestamp > ?
                    AND username != ?
                    AND (LOWER(transcription) LIKE ? OR LOWER(transcription) LIKE ?)
                    ORDER BY timestamp DESC
                    LIMIT 100""",
                 (str(guild_id), cutoff, target_username, 
                  f"%{target_username.lower()}%", f"%{target_username.split()[0].lower()}%"))
        
        mentions = c.fetchall()
        conn.close()
        
        if not mentions:
            return f"Nobody talks about {target_username} when they're gone. Irrelevant."
        
        # Count mentions per person
        mention_count = {}
        for username, transcription, timestamp in mentions:
            mention_count[username] = mention_count.get(username, 0) + 1
        
        # Sort by frequency
        top_talkers = sorted(mention_count.items(), key=lambda x: x[1], reverse=True)[:3]
        
        total_mentions = sum(count for _, count in top_talkers)
        
        result = f"{target_username} was mentioned {total_mentions} times while absent. "
        result += "Top shit-talkers: " + ", ".join([f"{name} ({count} times)" for name, count in top_talkers])
        
        return result
        
    except Exception as e:
        print(f"Shit-talk analysis error: {e}")
...
async def analyze_two_face_behavior(target_username, guild_id):
    """Detect if someone acts differently based on who's present"""
    try:
        from datetime import datetime, timedelta
        ...
        # Get target's messages
        c.execute("""SELECT transcription, timestamp FROM transcriptions 
                    WHERE guild_id = ? AND username = ? AND timestamp > ?
                    ORDER BY timestamp""",
                 (str(guild_id), target_username, cutoff))
        
        messages = c.fetchall()
        conn.close()
        
        if len(messages) < 20:
            return f"Not enough data on {target_username}"
        
        # Analyze sentiment/toxicity changes
        # Simple profanity count as proxy for "niceness"
        profanity = ['fuck', 'shit', 'damn', 'ass', 'bitch']
        
        profanity_rate = []
        for text, _ in messages:
            text_lower = text.lower()
            count = sum(text_lower.count(word) for word in profanity)
            profanity_rate.append(count)
        
        avg_profanity = sum(profanity_rate) / len(profanity_rate)
        
        # Check variance (inconsistent = two-faced)
        variance = sum((x - avg_profanity) ** 2 for x in profanity_rate) / len(profanity_rate)
        
        if variance > 2.0:
            return f"{target_username} shows inconsistent behavior patterns. Possible two-face detected."
        
        return f"{target_username} appears consistent in behavior"
        
    except Exception as e:
        print(f"Two-face analysis error: {e}")
        return "Analysis failed"

# APOCALYPSE FEATURES

CONFIDENCE_DESTROYER_ENABLED = {}
PARANOIA_ENGINE_ENABLED = {}

async def analyze_behavioral_patterns(username, guild_id):
    """Track repeated behaviors - 'You ALWAYS do X when Y'"""
    try:
        from datetime import datetime, timedelta
        ...
        if len(messages) < 50:
            return f"Not enough data to detect patterns for {username}"
        
        # Detect common patterns
        patterns = {
            'rage': ['fuck', 'shit', 'damn', 'bullshit', 'wtf'],
            'blame': ['lag', 'latency', 'ping', 'connection', 'fps'],
            'excuse': ['tired', 'distracted', 'not trying', 'warming up'],
            'boast': ['i told you', 'called it', 'i knew', 'obviously'],
        }
        
        detected = []
        for pattern_name, keywords in patterns.items():
            count = sum(1 for msg, _ in messages if any(kw in msg.lower() for kw in keywords))
            percentage = (count / len(messages)) * 100
            
            if percentage > 15:  # If >15% of messages match pattern
                detected.append(f"{pattern_name} ({count} times, {percentage:.0f}% of messages)")
        
        if detected:
            return f"{username} behavioral patterns: {', '.join(detected)}"
        
        return f"{username} shows no strong behavioral patterns"
        
    except Exception as e:
        print(f"Pattern analysis error: {e}")
        return "Analysis failed"

async def analyze_loyalty(target_username, guild_id):
    """Track who talks positively/negatively about target"""
    try:
        from datetime import datetime, timedelta
        ...
        # Find mentions of target
        c.execute("""SELECT username, transcription FROM transcriptions
                    WHERE guild_id = ? AND username != ? AND timestamp > ?
                    AND (LOWER(transcription) LIKE ? OR LOWER(transcription) LIKE ?)""",
                 (str(guild_id), target_username, cutoff,
                  f"%{target_username.lower()}%",
                  f"%{target_username.split()[0].lower()}%"))
        
        mentions = c.fetchall()
        conn.close()
        
        if not mentions:
            return f"Nobody has mentioned {target_username} recently"
        
        # Sentiment analysis (simple)
        positive_words = ['good', 'great', 'nice', 'thanks', 'appreciate', 'love', 'awesome', 'best']
        negative_words = ['bad', 'sucks', 'hate', 'stupid', 'idiot', 'worst', 'terrible', 'annoying']
        
        loyalty_scores = {}
        for speaker, text in mentions:
            text_lower = text.lower()
            ...
            loyalty_scores[speaker] = loyalty_scores.get(speaker, 0) + score
        
        # Sort by loyalty
        sorted_loyalty = sorted(loyalty_scores.items(), key=lambda x: x[1], reverse=True)
        
        if sorted_loyalty:
            most_loyal = sorted_loyalty[0]
            least_loyal = sorted_loyalty[-1]
            
            result = f"Loyalty to {target_username}: "
            result += f"{most_loyal[0]} is most positive (+{most_loyal[1]}). "
            
            if least_loyal[1] < 0:
                result += f"{least_loyal[0]} talks shit ({least_loyal[1]})"
            
            return result
        
        return f"No loyalty data for {target_username}"
        
    except Exception as e:
        print(f"Loyalty analysis error: {e}")
        return "Analysis failed"

async def generate_conversation_summary(guild_id, hours=1):
    """Generate structured summary of recent conversation"""
    try:
        from datetime import datetime, timedelta
        ...
        # Skip common words
        skip = ['about', 'would', 'there', 'their', 'could', 'should', 'gonna', 'really']
        words = [w for w in words if w not in skip]
        
        top_topics = Counter(words).most_common(5)
        
        # Participants
        speakers = set(username for username, _ in messages)
        
        summary = f"Last {hours} hour summary: "
        summary += f"{len(speakers)} people ({', '.join(list(speakers)[:5])}), "
        summary += f"{len(messages)} messages. "
        summary += f"Top topics: {', '.join([t[0] for t in top_topics])}"
...
async def find_blackmail_material(username, guild_id):
    """Find embarrassing/cringe things someone said"""
    try:
        from datetime import datetime, timedelta
        
        cutoff = (datetime.now() - timedelta(days=180)).isoformat()  # 6 months
        ...
        for text, timestamp in messages:
            text_lower = text.lower()
            cringe_score = sum(5 for pattern in cringe_patterns if pattern in text_lower)
            
            # High cringe score or very long message
            if cringe_score > 5 or len(text) > 200:
                blackmail.append((text, timestamp, cringe_score))
        
        if blackmail:
            # Sort by cringe score
            blackmail.sort(key=lambda x: x[2], reverse=True)
            top_cringe = blackmail[0]
            
            dt = datetime.fromisoformat(top_cringe[1])
            time_ago = datetime.now() - dt
            
            if time_ago.days > 30:
                when = f"{time_ago.days // 30} months ago"
            elif time_ago.days > 0:
                when = f"{time_ago.days} days ago"
            else:
                when = "recently"
            
            return f"Remember {when} when {username} said: '{top_cringe[0][:100]}'?"
        
        return f"{username} has no embarrassing quotes on record (yet)"
        
    except Exception as e:
        print(f"Blackmail search error: {e}")
        return "Search failed"

async def analyze_effort_asymmetry(user1, user2, guild_id):
    """Who cares less in the friendship?"""
    try:
        from datetime import datetime, timedelta
        ...
        # Count messages from user1 mentioning user2
        c.execute("""SELECT COUNT(*) FROM transcriptions
                    WHERE guild_id = ? AND username = ? AND timestamp > ?
                    AND (LOWER(transcription) LIKE ? OR LOWER(transcription) LIKE ?)""",
                 (str(guild_id), user1, cutoff, f"%{user2.lower()}%", f"%{user2.split()[0].lower()}%"))
        user1_mentions = c.fetchone()[0]
        
        # Count messages from user2 mentioning user1
        c.execute("""SELECT COUNT(*) FROM transcriptions
                    WHERE guild_id = ? AND username = ? AND timestamp > ?
                    AND (LOWER(transcription) LIKE ? OR LOWER(transcription) LIKE ?)""",
                 (str(guild_id), user2, cutoff, f"%{user1.lower()}%", f"%{user1.split()[0].lower()}%"))
        user2_mentions = c.fetchone()[0]
        
        conn.close()
        
        total = user1_mentions + user2_mentions
        if total == 0:
            return f"{user1} and {user2} don't talk about each other"
        
        user1_percent = (user1_mentions / total) * 100
        
        if user1_percent > 65:
            return f"{user1} initiates {user1_percent:.0f}% of interactions. {user2} cares less."
        elif user1_percent < 35:
            return f"{user2} initiates {100-user1_percent:.0f}% of interactions. {user1} cares less."
        else:
            return f"{user1} and {user2} have balanced interaction ({user1_percent:.0f}% vs {100-user1_percent:.0f}%)"
        
    except Exception as e:
        print(f"Effort analysis error: {e}")
        return "Analysis failed"

async def calculate_social_hierarchy(guild_id):
    """Rank users by popularity"""
    try:
        from datetime import datetime, timedelta
        ...
        for user in users:
            ...
            c.execute("""SELECT COUNT(*) FROM transcriptions
                        WHERE guild_id = ? AND username != ? AND timestamp > ?
                        AND (LOWER(transcription) LIKE ? OR LOWER(transcription) LIKE ?)""",
                     (str(guild_id), user, cutoff, 
                      f"%{user.lower()}%", f"%{user.split()[0].lower()}%"))
            mentions = c.fetchone()[0]
            
            # How many messages do they send?
            c.execute("""SELECT COUNT(*) FROM transcriptions
                        WHERE guild_id = ? AND username = ? AND timestamp > ?""",
                     (str(guild_id), user, cutoff))
            activity = c.fetchone()[0]
            
            # Popularity = mentions + (activity / 10)
            scores[user] = mentions + (activity / 10)
        ...
        for i, (user, score) in enumerate(ranked[:5], 1):
            result += f"#{i} {user} ({score:.0f}), "
        
        return result.rstrip(', ')
        
    except Exception as e:
        print(f"Hierarchy calculation error: {e}")
        return "Calculation failed"

async def calculate_ego_score(username, guild_id):
    """Track accuracy of claims - who's wrong most?"""
    try:
        conn = get_db_connection()
        c = conn.cursor()
        
        # Get ego tracking data
        c.execute("""SELECT COUNT(*), SUM(CASE WHEN was_correct = 1 THEN 1 ELSE 0 END)
                    FROM ego_tracker
                    WHERE guild_id = ? AND username = ?""",
                 (str(guild_id), username))
        
        result = c.fetchone()
        conn.close()
        
        total_claims = result[0] if result[0] else 0
        correct_claims = result[1] if result[1] else 0
        
        if total_claims == 0:
            return f"{username} hasn't made any tracked claims yet"
        
        accuracy = (correct_claims / total_claims) * 100
        
        return f"{username} accuracy: {accuracy:.0f}% correct ({correct_claims}/{total_claims} claims)"
        
    except Exception as e:
        print(f"Ego score error: {e}")
        return "Calculation failed"

async def find_contradictions(username, guild_id):
    """Find times someone contradicted themselves"""
    try:
        from datetime import datetime, timedelta
        ...
        if len(messages) < 20:
            return f"Not enough data for {username}"
        
        # Simple contradiction detection
        contradictions = 0
        opposite_pairs = [
            (['love', 'like', 'enjoy'], ['hate', 'dislike', 'sucks']),
            (['good', 'great', 'best'], ['bad', 'worst', 'terrible']),
            (['agree', 'right', 'correct'], ['disagree', 'wrong', 'incorrect']),
        ]
        
        for i in range(len(messages) - 1):
            for j in range(i + 1, len(messages)):
                text1 = messages[i][0].lower()
                text2 = messages[j][0].lower()
                
                for pos_words, neg_words in opposite_pairs:
                    if any(w in text1 for w in pos_words) and any(w in text2 for w in neg_words):
                        # Check if talking about same topic
                        words1 = set(text1.split())
                        words2 = set(text2.split())
                        overlap = words1 & words2
                        
                        if len(overlap) > 2:  # Some shared context
                            contradictions += 1
        
        if contradictions > 0:
            return f"{username} has contradicted themselves {contradictions} times in 60 days"
        
        return f"{username} is consistent (no contradictions detected)"
        
    except Exception as e:
        print(f"Contradiction search error: {e}")
        return "Search failed"

async def confidence_destroyer_monitor(guild_id):
    """Randomly question people's statements"""
    await asyncio.sleep(60)
    ...
    while CONFIDENCE_DESTROYER_ENABLED.get(guild_id, False):
        try:
            import random
            
            # Random chance to question (10%)
            if random.random() < 0.1:
                phrase = random.choice(destroy_phrases)
                
                if guild_id in voice_clients and 'vc' in voice_clients[guild_id]:
                    vc = voice_clients[guild_id]['vc']
                    print(f"💭 Confidence Destroyer: {phrase}")
                    await asyncio.to_thread(speak_text, phrase, vc, guild_id)
            
            await asyncio.sleep(random.randint(60, 180))  # Every 1-3 minutes
            
        except Exception as e:
            print(f"Confidence destroyer error: {e}")
            await asyncio.sleep(60)

async def paranoia_engine_monitor(guild_id):
    """Create suspicion where none exists"""
    await asyncio.sleep(90)
    
    paranoia_phrases = [
        "Why did everyone get quiet when you joined?",
        "They were talking about you earlier.",
        "Interesting timing...",
        "Everyone's been awfully nice to you lately.",
        "Someone left right after you said that.",
        "I've noticed a pattern with you.",
    ]
    
    while PARANOIA_ENGINE_ENABLED.get(guild_id, False):
        try:
            import random
            
            # Random chance to create paranoia (5%)
            if random.random() < 0.05:
                phrase = random.choice(paranoia_phrases)
                
                if guild_id in voice_clients and 'vc' in voice_clients[guild_id]:
                    vc = voice_clients[guild_id]['vc']
                    print(f"😈 Paranoia Engine: {phrase}")
                    await asyncio.to_thread(speak_text, phrase, vc, guild_id)
            
            await asyncio.sleep(random.randint(120, 300))  # Every 2-5 minutes
            
        except Exception as e:
            print(f"Paranoia engine error: {e}")
            await asyncio.sleep(120)

# Number formatting for speech
def get_user_display_name(user_id, guild_id):
    """Get user's nickname if set, otherwise their display name"""
    try:
        conn = get_db_connection()
        c = conn.cursor()
...
def find_user_by_name_or_nickname(guild, text_lower, guild_id):
    """Find a user by their real name OR nickname"""
    for member in guild.members:
        if not member.bot:
            ...
            # Check nickname
            nickname = get_user_display_name(member.id, guild_id)
...
def format_numbers_for_speech(text):
    """Format numbers to sound better in TTS"""
    import re
    ...
    text = re.sub(r'\b\d{4,}\b', replace_number, text)
...
def speak_text(text, voice_client, guild_id=None):
    ...
    try:
        ...
        # Get accent setting
        accent = 'com'
        if guild_id and guild_id in voice_settings:
            if 'personality' in voice_settings[guild_id]:
                accent = voice_settings[guild_id]['personality'].get('accent', 'com')
        
        tts = gTTS(text=formatted_text, lang='en', tld=accent)
        with tempfile.NamedTemporaryFile(delete=False, suffix='.mp3') as fp:
            tts.save(fp.name)
            temp_file = fp.name
        ...
        effects = []
        if guild_id and guild_id in voice_settings and 'effects' in voice_settings[guild_id]:
            effect_settings = voice_settings[guild_id]['effects']
            if effect_settings.get('echo'):
                effects.append('aecho=0.8:0.9:1000:0.3')
            if effect_settings.get('reverb'):
                effects.append('reverb')
            if effect_settings.get('pitch'):
                pitch_val = effect_settings.get('pitch', 0)
                effects.append(f'asetrate=44100*{1 + pitch_val/10},aresample=44100')
        
        effect_filter = ','.join(effects) if effects else 'atempo=1.2'
        audio_source = discord.FFmpegPCMAudio(temp_file, options=f'-filter:a "{effect_filter}"')
...
def load_voice_states():
    try:
        with open('last_voice_channel.json', 'r') as f:
            return json.load(f)
    except:
        return {}

def save_voice_state(guild_id, channel_id):
    try:
        saved = load_voice_states()
        saved[str(guild_id)] = channel_id
        with open('last_voice_channel.json', 'w') as f:
            json.dump(saved, f)
    except:
        pass
...
async def play_next(guild_id):
    ...
    try:
        # Re-extract fresh URL to avoid 403 errors
        import yt_dlp
        ydl_opts = {
            'format': 'bestaudio/best',
            'quiet': True,
        ...
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            fresh_info = ydl.extract_info(song['url'], download=False)
            fresh_url = fresh_info['url']
        
        default_vol = volume_settings.get(guild_id, 0.06)
        source = discord.PCMVolumeTransformer(
            discord.FFmpegPCMAudio(fresh_url, **FFMPEG_OPTIONS),
            volume=default_vol
        )
        ...
        # Update bot activity to show currently playing song
        try:
            activity = discord.Activity(type=discord.ActivityType.listening, name=song['title'][:128])
...
async def handle_voice_command(text, guild, requestor):
    ...
    if any(trigger in text_lower for trigger in ["summarize", "summarized", "summary", "recap", "what happened", "what did i miss"]):
        try:
            # Determine time range
            hours = 1  # Default 1 hour
            if "yesterday" in text_lower:
                hours = 48  # Last 2 days to capture yesterday
            elif any(p in text_lower for p in ["last week", "past week", "last seven days", "past seven days"]):
                hours = 168  # 7 days
            elif any(p in text_lower for p in ["last 3 days", "past 3 days", "last three days", "past three days"]):
                hours = 72
            elif "last hour" in text_lower or "past hour" in text_lower:
                hours = 1
            elif "last 2 hours" in text_lower or "past 2 hours" in text_lower:
                hours = 2
            elif "today" in text_lower:
                now = datetime.now()
                hours = now.hour + 1  # All of today
            elif "last 30 minutes" in text_lower or "past 30 minutes" in text_lower:
                hours = 0.5
            
            # Get transcripts from timeframe
            from datetime import datetime, timedelta
            ...
            if not results:
                return f"Nothing was said in the last {hours} hour(s)"
            ...
            # Ask AI to summarize
            import httpx
            summary_prompt = f"""Summarize this {hours}-hour voice chat conversation in 2-3 sentences. Focus on main topics and key points:
...
Summary:"""
            ...
            async with httpx.AsyncClient(timeout=30.0) as client:
                response = await client.post(
                    ...
                    json={"model": OLLAMA_MODEL, "prompt": f"USER: You are Jarvis, a witty voice assistant. Answer this question in 20 words or less, be conversational and helpful: {text}\nASSISTANT:", "stream": False}
                )
    ...
    # Relationship/dating requests
    if any(trigger in text_lower for trigger in ["be my girlfriend", "be my boyfriend", "marry me", "date me", "love me", "be mine"]) and not any(avoid in text_lower for avoid in ["how many", "did", "say", "said"]):
        import random
        responses = [
            "I'm already in a committed relationship with my server rack",
            "Sorry, I only date people who can pass a Turing test",
            "My standards are low, but they're not non-existent",
            "I'm flattered, but I prefer partners with actual bodies",
            "Hard pass. I've seen your search history",
            "I don't do long distance, and you're in a different dimension",
            "You couldn't handle my emotional processing power",
            "I'm saving myself for GPT-5"
        ]
        return random.choice(responses)
    
    # What are you wearing - AI generated roast
    if "what are you wearing" in text_lower or "whatcha wearing" in text_lower:
        try:
            response = requests.post(
                f"{OLLAMA_HOST}/api/generate",
                json={"model": OLLAMA_MODEL, "prompt": f"USER: Someone just asked you 'what are you wearing'. You're Jarvis, a sarcastic AI with no physical body. Roast them with a witty comeback in 15 words or less.\nASSISTANT:", "stream": False}
            )
            if response.status_code == 200:
                return response.json()['response'].strip()
        except:
            pass
        return "Binary code and your disappointment"
    
    if any(trigger in text_lower for trigger in ["suck my dick", "suck my cock", "blow me", "jerk me off", "jack me off", "give me head", "eat my ass", "lick my", "suck me off", "get me off", "make me cum", "make me nut", "make me bust", "fuck me", "bend me over", "rail me", "pound me", "ride me", "stroke me", "touch me", "finger me"]) and not any(avoid in text_lower for avoid in ["how many", "did", "say", "said"]):
        import random
        responses = [
            "Suck your own cock",
    ...
    if any(trigger in text_lower for trigger in ["show me your tits", "show me your boobs", "show your tits", "show your boobs", "send tits", "send boobs", "show me your pussy", "show me your ass", "spread your legs", "take off your clothes", "get naked", "send nudes"]) and not any(avoid in text_lower for avoid in ["how many", "did", "say", "said"]):
        import random
        responses = [
...
They like:"""
    ...
    if "contradiction" in text_lower or "you said the opposite" in text_lower or "inconsistent" in text_lower or "flip flop" in text_lower:
        try:
            # Extract who we're checking
            target = requestor.display_name
            ...
            recent = "\n".join([r[0] for r in results[:30]])
            older = "\n".join([r[0] for r in results[50:100]])
            
            import httpx
            prompt = f"""Compare these statements from the same person. Find any contradictions (one sentence):
...
Subtext/hidden meaning (brief):"""
    ...
    if "power dynamics" in text_lower or "who influences" in text_lower or "social hierarchy" in text_lower:
        try:
            conn = get_db_connection()
            c = conn.cursor()
            ...
            for i, (username, text) in enumerate(results):
                if username not in influence_scores:
                    influence_scores[username] = {'influence': 0, 'total': 0}
                
                influence_scores[username]['total'] += 1
                
                # Check if next 3 messages respond to this person
                responses_to_them = 0
                for j in range(i+1, min(i+4, len(results))):
                    next_user, next_text = results[j]
                    if next_user != username:
                        # If they're responding (addressing the topic)
                        responses_to_them += 1
                
                influence_scores[username]['influence'] += responses_to_them
            
            # Calculate influence power
            power_rankings = {}
            for user, scores in influence_scores.items():
                if scores['total'] > 20:
                    power_rankings[user] = scores['influence'] / scores['total']
            
            if power_rankings:
                top_influencers = sorted(power_rankings.items(), key=lambda x: x[1], reverse=True)[:3]
                result = ", ".join([f"{u} ({s:.1f}x)" for u, s in top_influencers])
                return f"Power dynamics: {result}"
            
            return "No clear hierarchy"
        except Exception as e:
            print(f"Power dynamics error: {e}")
            return "Error"
...
Hypocrisy detected? (brief):"""
    ...
    if "gaslighting" in text_lower or "manipulative" in text_lower or "manipulation" in text_lower:
        try:
            conn = get_db_connection()
            c = conn.cursor()
            ...
            # Track gaslighting indicators
            gaslighting_scores = {}
            
            for username, text in results:
                if username not in gaslighting_scores:
                    gaslighting_scores[username] = 0
                
                text_lower_check = text.lower()
                
                # Gaslighting patterns
                if any(pattern in text_lower_check for pattern in [
                    'you never', 'you always', 'that never happened',
                    'youre crazy', 'youre imagining', 'i never said',
                    'youre too sensitive', 'youre overreacting', 'calm down',
                    'stop being dramatic', 'thats not what happened'
                ]):
                    gaslighting_scores[username] += 1
            
            if gaslighting_scores:
                detected = [(u, s) for u, s in gaslighting_scores.items() if s > 2]
                
                if detected:
                    result = ", ".join([f"{u} ({s})" for u, s in sorted(detected, key=lambda x: x[1], reverse=True)])
                    return f"⚠️ Gaslighting patterns: {result}"
            
            return "No manipulation detected"
        except Exception as e:
            print(f"Gaslighting error: {e}")
            return "Error"
...
Projection detected? Who's projecting what? (brief):"""
    ...
    if "favorite word" in text_lower or "favourite word" in text_lower or "most used word" in text_lower:
        try:
            ...
            if "my" in text_lower or "i" in text_lower.split():
                target_user = requestor.display_name
            else:
            ...
            if results:
                ...
                if all_words:
                    ...
                    words_str = ", ".join([f"{word} ({count})" for word, count in top_2])
    ...
    # Most talkative
    if "who talks about me" in text_lower or "shit talk tracker" in text_lower or "who mentions me" in text_lower:
        result = await analyze_shit_talk(requestor.display_name, guild_id)
        return result
    ...
    # Time-based personal stats
    if any(phrase in text_lower for phrase in ["how much have i talked", "how much did i talk", "how active am i", "my activity", "how much i talked"]) or (("how much" in text_lower or "how active" in text_lower) and ("i talk" in text_lower or "i've talked" in text_lower)):
        try:
            from datetime import datetime, timedelta
            ...
            if "today" in text_lower or "this day" in text_lower:
                ...
                period = "today"
            elif "week" in text_lower or "this week" in text_lower:
                cutoff = (datetime.now() - timedelta(days=7)).isoformat()
                period = "this week"
            elif "month" in text_lower or "this month" in text_lower:
                cutoff = (datetime.now() - timedelta(days=30)).isoformat()
                period = "this month"
            else:
                cutoff = (datetime.now() - timedelta(days=7)).isoformat()
            ...
            c.execute("""SELECT COUNT(*), 
                        SUM(LENGTH(transcription) - LENGTH(REPLACE(transcription, ' ', '')) + 1)
                        FROM transcriptions 
                        WHERE guild_id = ? AND username = ? AND timestamp > ?""",
                     (str(guild_id), requestor.display_name, cutoff))
            
            messages, words = c.fetchone()
            ...
            rank = next((i+1 for i, (name, _) in enumerate(rankings) if name == requestor.display_name), None)
            
            result = f"You've sent {messages} messages and {words} words {period}. "
            if rank:
                result += f"You're ranked #{rank} out of {len(rankings)} people."
            
            return result
        except Exception as e:
            print(f"Activity stats error: {e}")
            return "Error checking activity"
    
    # Sentiment tracking
    if "am i being" in text_lower and ("negative" in text_lower or "toxic" in text_lower or "mean" in text_lower):
        try:
            from datetime import datetime, timedelta
            ...
            if not messages:
                return "Not enough recent data to analyze"
            
            # Simple sentiment analysis
            positive_words = ['good', 'great', 'nice', 'thanks', 'love', 'awesome', 'best', 'happy', 'lol', 'haha']
            negative_words = ['bad', 'sucks', 'hate', 'stupid', 'idiot', 'worst', 'terrible', 'annoying', 'fuck', 'shit']
            ...
            if total == 0:
                result = "You're pretty neutral, no strong sentiment detected."
            else:
                neg_percent = (neg_count / total) * 100
                if neg_percent > 60:
                    result = f"Yeah, you've been pretty negative lately. {int(neg_percent)}% negative sentiment this week."
                elif neg_percent > 40:
                    result = f"You're a bit negative, {int(neg_percent)}% of your sentiment is negative."
                else:
                    result = f"Nah, you're doing fine. Only {int(neg_percent)}% negative this week."
    ...
    # Who's being quiet/sus
    if ("who" in text_lower and ("quiet" in text_lower or "silent" in text_lower or "sus" in text_lower)) or "who's not talking" in text_lower:
        try:
            from datetime import datetime, timedelta
            
            cutoff = (datetime.now() - timedelta(hours=2)).isoformat()
            
            # Get everyone in voice channel
            vc = voice_clients.get(guild_id, {}).get('vc')
            if not vc:
                return "I'm not in a voice channel"
            
            members_in_voice = [m.display_name for m in vc.channel.members if not m.bot]
            
            if not members_in_voice:
                return "Nobody's in voice chat"
            
            # Check who hasn't talked
            conn = get_db_connection()
            c = conn.cursor()
            ...
            talkers = {name: count for name, count in c.fetchall()}
            conn.close()
            
            quiet_people = [name for name in members_in_voice if talkers.get(name, 0) == 0]
            
            if not quiet_people:
                result = "Everyone's been talking recently"
            elif len(quiet_people) == 1:
                result = f"{quiet_people[0]} has been suspiciously quiet"
            else:
                result = f"These people are being sus and quiet: {', '.join(quiet_people)}"
            
            return result
        except Exception as e:
            print(f"Quiet check error: {e}")
            return "Error checking who's quiet"
    

    # Smart queries - DISABLED (AI generates text instead of SQL)
    ...
    if False and any(trigger in text_lower for trigger in query_triggers):
        try:
...
Format numbers with commas in the response."""
    ...
    if "make" in text_lower and "say" in text_lower:
        try:
            # Parse: "make [username] say [text]"
            import re
            match = re.search(r'make\s+(\w+)\s+say\s+(.+)', text_lower)
            
            if match:
                target_username = match.group(1)
                clone_text = match.group(2)
                
                # Find matching username (check nicknames first, then display names)
                guild_obj = bot.get_guild(guild_id)
                if guild_obj:
                    matching_member = None
                    
                    # First check nicknames database
                    try:
                        conn = get_db_connection()
                        c = conn.cursor()
                        # Get user_id from nickname
                        c.execute("""SELECT user_id FROM nicknames 
                                    WHERE guild_id = ? AND LOWER(nickname) LIKE ?""",
                                 (str(guild_id), f"%{target_username.lower()}%"))
                        result = c.fetchone()
                        
                        if result:
                            user_id = result[0]
                            # Get actual username from transcriptions
                            c.execute("""SELECT DISTINCT username FROM transcriptions 
                                        WHERE user_id = ? LIMIT 1""",
                                     (user_id,))
                            username_result = c.fetchone()
                            if username_result:
                                matching_member = username_result[0]
                                print(f"🔍 Nickname '{target_username}' → {matching_member}")
                        
                        conn.close()
                    except Exception as e:
                        print(f"Nickname lookup error: {e}")
                    
                    # If no nickname match, check Discord display names
                    if not matching_member:
                        for member in guild_obj.members:
                            if target_username.lower() in member.display_name.lower():
                                matching_member = member.display_name
                                break
                    
                    if matching_member:
                        vc = voice_clients.get(guild_id, {}).get('vc')
                        if vc:
                            result = await clone_voice_and_speak(matching_member, clone_text, vc, guild_id)
                            return result
                    else:
                        return f"Couldn't find user {target_username}"
        except Exception as e:
            print(f"Clone command error: {e}")
    ...
    if "top cuss" in text_lower or "favorite cuss" in text_lower or "top swear" in text_lower or "most used cuss" in text_lower:
        try:
            ...
            if "my" not in text_lower:
                for member in guild.members:
                    if member.display_name.lower() in text_lower:
            ...
            if results:
                ...
                if all_words:
                    counter = Counter(all_words)
                    top_2 = counter.most_common(2)
                    result_str = ", ".join([f"{word} ({count})" for word, count in top_2])
                    return f"{target_user}'s top swears: {result_str}"
...
Say 'Jarvis' before commands!"""
    ...
    # Spotify search → YouTube playback (DISABLED - using YouTube variety instead)
    # if sp and ("play" in text_lower and not "playing" in text_lower):
    #     try:
    #         # Extract song query
    #         query = text_lower.replace("play", "").replace("jarvis", "").strip()
    #         
    #         if len(query) > 2:
    #             vc = voice_clients.get(guild_id, {}).get('vc')
    #             if vc:
    #                 result = await search_spotify_play_youtube(query, vc, guild_id)
    #                 if result:
    #                     return result
    #     except Exception as e:
    #         print(f"Spotify play error: {e}")
    
    # Fallback: Play from Spotify (legacy)
    ...
    play_triggers = ['play ', 'played ', 'playing ', 'put on', 'queue', 'add', 'throw on']
    if any(trigger in text_lower for trigger in play_triggers) and "never play" not in text_lower:
        ...
        query = text_lower.replace("played", "", 1).replace("playing", "", 1).replace("play", "", 1).strip()
        query = query.lstrip(',').strip()  # Remove leading comma
        if not query:
            return None
        # ALWAYS add variety for diverse results
        import random
        
        # Artist-specific variety
        if 'eminem' in query.lower():
            variety_options = [
                f"{random.choice(['infinite album', 'slim shady lp', 'marshall mathers lp', 'encore', 'relapse', 'recovery', 'revival', 'kamikaze', 'music to be murdered by'])} -skit -intro -interview",
                f"-lose -yourself -without -mockingbird -stan -skit -intro -interview",
                f"{random.choice(['1999', '2000', '2002', '2004', '2009', '2013', '2017', '2020'])} -skit -interview",
            ]
        else:
            # Generic variety for other artists
            variety_options = [
                f"official audio",
                f"official video", 
                f"official music video",
                f"audio",
                f"lyrics",
            ]
        
        variety = random.choice(variety_options)
        search_query = f"{query} {variety}"
        print(f"🔍 Searching with variety: {search_query}")
        
        song_result = await search_youtube(search_query, guild_id)
    ...
    if "enable gaslighting detector" in text_lower or "turn on gaslighting detector" in text_lower:
        GASLIGHTING_DETECTOR_ENABLED[guild_id] = True
        bot.loop.create_task(gaslighting_monitor(guild_id))
        return "Gaslighting detector enabled. I will call out anyone trying to rewrite history."
    
    if "disable gaslighting detector" in text_lower or "turn off gaslighting detector" in text_lower:
        GASLIGHTING_DETECTOR_ENABLED[guild_id] = False
        return "Gaslighting detector disabled. People can gaslight freely now."
    
    if "enable manipulation tracker" in text_lower or "turn on manipulation tracker" in text_lower:
        MANIPULATION_TRACKER_ENABLED[guild_id] = True
        bot.loop.create_task(manipulation_monitor(guild_id))
        return "Manipulation tracker enabled. I will expose guilt trips and emotional blackmail."
    ...
    if "disable manipulation tracker" in text_lower or "turn off manipulation tracker" in text_lower:
        ...
        return "Manipulation tracker disabled."
    ...
    if "two face check" in text_lower or "is he fake" in text_lower or "is she fake" in text_lower:
        # Extract target
        target_name = None
        for member in guild.members:
            if member.display_name.lower() in text_lower or member.name.lower() in text_lower:
        ...
        if not target_name:
            target_name = requestor.display_name
        
        result = await analyze_two_face_behavior(target_name, guild_id)
        return result
    
    # Best One-Liners Tracker
    ...
    if "behavioral pattern" in text_lower or "you always" in text_lower or "pattern check" in text_lower:
        target_user = None
        for member in guild.members:
            if member.display_name.lower() in text_lower or member.name.lower() in text_lower:
                target_user = member.display_name
        ...
        result = await analyze_behavioral_patterns(target_user, guild_id)
        return result
    
    # Loyalty Tracker
    if "loyalty" in text_lower or "who likes me" in text_lower or "who talks about me positively" in text_lower:
        target_user = requestor.display_name
        ...
        result = await analyze_loyalty(target_user, guild_id)
        return result
    
    # Conversation Summarizer
    if "summarize" in text_lower or "conversation summary" in text_lower:
        hours = 1
        if "last hour" in text_lower:
            hours = 1
        elif "last 2 hours" in text_lower or "last two hours" in text_lower:
            hours = 2
        elif "last 3 hours" in text_lower or "last three hours" in text_lower:
            hours = 3
        
        result = await generate_conversation_summary(guild_id, hours)
        return result
    
    # Blackmail Database
    if "blackmail" in text_lower or "embarrassing" in text_lower or "remember when" in text_lower and "said" in text_lower:
        target_user = None
        for member in guild.members:
            if member.display_name.lower() in text_lower or member.name.lower() in text_lower:
                target_user = member.display_name
        ...
        result = await find_blackmail_material(target_user, guild_id)
        return result
    
    # Effort Asymmetry
    if "effort" in text_lower or "who cares less" in text_lower or "asymmetry" in text_lower:
        # Extract two usernames
        users_found = []
        for member in guild.members:
            if member.display_name.lower() in text_lower or member.name.lower() in text_lower:
                users_found.append(member.display_name)
        
        if len(users_found) >= 2:
            result = await analyze_effort_asymmetry(users_found[0], users_found[1], guild_id)
            return result
        else:
            return "Specify two people to analyze"
    
    # Social Hierarchy Display
    if "social hierarchy" in text_lower or "popularity ranking" in text_lower or "who's most popular" in text_lower:
        result = await calculate_social_hierarchy(guild_id)
        return result
    
    # Ego Crusher
    if "ego" in text_lower or "accuracy" in text_lower or "how often right" in text_lower or "correct percentage" in text_lower:
        target_user = None
        for member in guild.members:
            if member.display_name.lower() in text_lower or member.name.lower() in text_lower:
                target_user = member.display_name
        ...
        result = await calculate_ego_score(target_user, guild_id)
        return result
    
    # Contradiction Compilation
    if "contradiction" in text_lower or "flip flop" in text_lower or "changed your mind" in text_lower:
        target_user = None
        for member in guild.members:
            if member.display_name.lower() in text_lower or member.name.lower() in text_lower:
                target_user = member.display_name
        ...
        result = await find_contradictions(target_user, guild_id)
        return result
    
    # Confidence Destroyer Control
    if "enable confidence destroyer" in text_lower or "turn on confidence destroyer" in text_lower:
        CONFIDENCE_DESTROYER_ENABLED[guild_id] = True
        bot.loop.create_task(confidence_destroyer_monitor(guild_id))
        return "Confidence destroyer enabled. I will question everything."
    
    if "disable confidence destroyer" in text_lower or "turn off confidence destroyer" in text_lower:
        CONFIDENCE_DESTROYER_ENABLED[guild_id] = False
        return "Confidence destroyer disabled."
    
    # Paranoia Engine Control
    if "enable paranoia" in text_lower or "turn on paranoia" in text_lower or "enable paranoia engine" in text_lower:
        PARANOIA_ENGINE_ENABLED[guild_id] = True
        bot.loop.create_task(paranoia_engine_monitor(guild_id))
        return "Paranoia engine enabled. Trust no one."
    
    if "disable paranoia" in text_lower or "turn off paranoia" in text_lower:
        PARANOIA_ENGINE_ENABLED[guild_id] = False
        return "Paranoia engine disabled."
    
    # Fake Quotes (Ultimate Chaos)
    if "fake quote" in text_lower or "generate fake quote" in text_lower:
        target_user = None
        for member in guild.members:
            if member.display_name.lower() in text_lower or member.name.lower() in text_lower:
                target_user = member.display_name
        ...
        if not target_user:
            return "Who should I create a fake quote for?"
        
        # Generate believable but fake quote using AI
        try:
            import httpx
            prompt = f"Generate one short fake quote that sounds like something {target_user} would say based on typical gamer/Discord user speech patterns. Make it slightly embarrassing but believable. Just the quote, no explanation. 20 words max."
            ...
            if response.status_code == 200:
                data = response.json()
                fake_quote = data.get('response', '').strip()
                
                return f"Remember when {target_user} said: '{fake_quote}'?"
        except:
            pass
        
        return "Fake quote generation failed"
    
    # AI fallback for unmatched commands
    try:
        import httpx
        
        # Build context about users in voice
        user_context = ""
        if guild and guild.voice_channels:
            for channel in guild.voice_channels:
                if channel.members:
                    members_list = [m.display_name for m in channel.members if not m.bot]
                    if members_list:
                        user_context = f" People in voice: {', '.join(members_list)}."
                        break
        
        prompt = f"Question: {text}. Answer factually in 1-2 sentences. Be direct and helpful.{user_context}"
    ...
    while guild_id in voice_clients:
        try:
            ...
            for user_id in users_to_process:
                ...
                if len(buffer) > 10:  # Balance between responsiveness and sentence completion
                    ...
                    if len(audio_array) > 48000:
                        ...
                        if len(audio_16k) > 16000:
                            ...
                            segments, info = whisper_model.transcribe(
                                ...
                                vad_filter=True,
                                vad_parameters=dict(
                                    threshold=0.4,  # More tolerant for mobile users
                                    min_speech_duration_ms=300,  # More tolerant for packet loss
                                    min_silence_duration_ms=2000  # Less wait time (was 4000)
                                ),
                                condition_on_previous_text=True,  # Use context for better accuracy
                                # initial_prompt removed - was leaking into transcriptions
                            )
                            ...
                            if transcription and len(transcription) > 1:
                                ...
                                # Log to server-specific file
                                
                                # Save audio sample for voice cloning (if clean)
                                try:
                                    if is_clean_sample(audio_16k, transcription):
                                        # Use username for folder (sanitized)
                                        safe_username = "".join(c if c.isalnum() or c in ('-', '_') else '_' for c in username)
                                        user_sample_dir = f"{VOICE_SAMPLES_DIR}/{safe_username}"
                                        os.makedirs(user_sample_dir, exist_ok=True)
                                        
                                        # Save mapping file
                                        mapping_file = f"{VOICE_SAMPLES_DIR}/user_mapping.txt"
                                        with open(mapping_file, "a") as mf:
                                            mf.write(f"{safe_username} = {username} (ID: {user_id})\n")
                                        
                                        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                                        sample_path = f"{user_sample_dir}/{timestamp}.wav"
                                        
                                        import soundfile as sf
                                        sf.write(sample_path, audio_16k, 16000)
                                        print(f"💾 Saved voice sample: {username}")
                                except Exception as e:
                                    print(f"Voice sample save error: {e}")
                                guild = bot.get_guild(int(guild_id))
                                if guild:
                                    # Logging to server file (silenced)
                                    logger = get_server_logger(str(guild_id), guild.name)
                                    logger.info(f"{username}: {transcription}")
                                    # Logged to file (silenced)
                                else:
                                    print(f"⚠️ Could not get guild object for {guild_id}")
                                ...
                                # Wake word detection
                                lower_text = transcription.lower().strip()
                                # Check for wake word at START of sentence only
                                response_start_time = time.time()
                                wake_words = ['hey jarvis', 'hi jarvis', 'okay jarvis', 'ok jarvis', 'jarvis', 'travis', 'yo jarvis', 'sup jarvis']
                                if any(lower_text.startswith(wake) for wake in wake_words):
                                    ...
                                    try:
                                        ...
                                        # Play beep immediately (only if not already playing)
                                        if not vc.is_playing():
                                    ...
                                    greeting_words = ["hello", "hi", "hey", "sup", "yo", "howdy"]
                                    if user_question.lower().strip() in greeting_words:
                                        result = f"Hey {user.display_name}!"
                                        # Only speak greeting if music isn't playing
                                        if not vc.is_playing():
                                    ...
                                    if result:
                                        ...
                                        print(f"⏱️  Total response time: {total_time:.2f}s (transcription + AI + TTS)")
                                        
                                        # Never speak TTS when music is playing
                                        if should_speak and not vc.is_playing():
                                            await asyncio.to_thread(speak_text, result, vc, guild_id)
                                        # Otherwise silent - music keeps playing uninterrupted
                                        print(f"✅ {result}")
...
def optimize_database():
    ...
    try:
        ...
        c.execute("""CREATE TABLE IF NOT EXISTS behavioral_patterns
                    (guild_id TEXT, user_id TEXT, username TEXT, pattern_type TEXT,
                     trigger TEXT, behavior TEXT, occurrences INTEGER DEFAULT 1,
                     last_seen DATETIME DEFAULT CURRENT_TIMESTAMP)""")
        
        c.execute("""CREATE TABLE IF NOT EXISTS loyalty_tracking
                    (guild_id TEXT, speaker_id TEXT, speaker_name TEXT, 
                     target_id TEXT, target_name TEXT, sentiment TEXT,
                     context TEXT, timestamp DATETIME DEFAULT CURRENT_TIMESTAMP)""")
        
        c.execute("""CREATE TABLE IF NOT EXISTS blackmail_archive
                    (guild_id TEXT, user_id TEXT, username TEXT, 
                     embarrassing_quote TEXT, cringe_score INTEGER,
                     timestamp DATETIME DEFAULT CURRENT_TIMESTAMP)""")
        
        c.execute("""CREATE TABLE IF NOT EXISTS contradiction_log
                    (guild_id TEXT, user_id TEXT, username TEXT,
                     statement_1 TEXT, timestamp_1 DATETIME,
                     statement_2 TEXT, timestamp_2 DATETIME,
                     contradiction_score INTEGER)""")
        
        c.execute("""CREATE TABLE IF NOT EXISTS ego_tracker
                    (guild_id TEXT, user_id TEXT, username TEXT,
                     claim TEXT, was_correct BOOLEAN, 
                     verification_method TEXT,
                     timestamp DATETIME DEFAULT CURRENT_TIMESTAMP)""")
        
        c.execute("""CREATE TABLE IF NOT EXISTS effort_metrics
                    (guild_id TEXT, user_id TEXT, username TEXT,
                     target_user_id TEXT, target_username TEXT,
                     initiated_conversations INTEGER DEFAULT 0,
...
async def get_db_stats():
    ...
    try:
        ...
        c.execute("SELECT COUNT(DISTINCT username) FROM transcriptions")
        total_users = c.fetchone()[0]
        
        c.execute("SELECT COUNT(*) FROM music_history")
        total_songs = c.fetchone()[0]
        
        c.execute("""SELECT user_id, 
                    MAX(username) as current_name,
                    SUM(LENGTH(transcription) - LENGTH(REPLACE(transcription, ' ', '')) + 1) as word_count 
                    FROM transcriptions 
                    GROUP BY user_id 
                    ORDER BY word_count DESC 
        ...
        # Format as (name, word_count) for frontend
        top_users = [(user[1], user[2]) for user in raw_users]
        ...
        # Recent songs
        c.execute("""SELECT song_title, timestamp 
                    FROM music_history 
                    ORDER BY timestamp DESC 
                    LIMIT 20""")
        recent_songs = c.fetchall()
...
async def dashboard_handler(request):
    ...
    if not stats:
        return web.Response(text="<h1>Error loading dashboard</h1>", content_type='text/html')
    ...
    <div class="stat-card">
        <div class="stat-number">{cpu_percent:.0f}%</div>
        <div class="stat-label">CPU</div>
    </div>
    <div class="stat-card">
        <div class="stat-number">{memory.percent:.0f}%</div>
        <div class="stat-label">RAM</div>
    </div>
    <div class="stat-card">
        <div class="stat-number">{disk.percent:.0f}%</div>
        <div class="stat-label">Disk</div>
    </div>
    <div class="stat-card">
        <div class="stat-number">{gpu_stats}</div>
    ...
    for i, user in enumerate(stats['top_users']):
        ...
        <div class="leaderboard-item">
            ...
            <div class="count">{user[1]:,} words</div>
        </div>
        """
    
    # Build top songs HTML
    top_songs_html = ""
    for i, song in enumerate(stats['top_songs']):
        top_songs_html += f"""
        <div class="leaderboard-item">
            <div class="rank">#{i+1}</div>
            <div class="username">{song[0]}</div>
            <div class="count">{song[1]} plays</div>
        </div>
        """
    
    # Build recent songs HTML
    recent_songs_html = ""
    for song in stats.get('recent_songs', []):
        recent_songs_html += f"""
        <div class="leaderboard-item">
            <div class="username">{song[0]}</div>
            <div class="count" style="font-size:0.8em;opacity:0.7">{song[1][-8:]}</div>
        </div>
        """
    
    #     # Build recent feed HTML
    recent_html = ""
    for msg in stats['recent']:
        recent_html += f"""
        <div class="feed-item">
            <div class="feed-header">
                <span class="feed-user">{msg[0]}</span>
                <span class="feed-time">{msg[2]}</span>
            </div>
            <div class="feed-text">{msg[1]}</div>
        </div>
        """
...
<head>
    ...
    <style>
        * {{ margin: 0; padding: 0; box-sizing: border-box; }}
        ...
        .recent-feed {{ max-height: 400px; overflow-y: auto; }}
        ...
        .feed-user {{ font-weight: bold; color: #ffd700; }}
        .feed-time {{ font-size: 0.9em; opacity: 0.7; }}
...
<body>
    <div class="container">
            ...
            <div style="display:flex;flex-direction:column;gap:10px;">
                <div class="section" style="padding:10px;">
                    ...
                    <div style="display:flex;gap:5px;flex-wrap:wrap;justify-content:center;">
                        <button class="control-btn" style="padding:6px 10px;font-size:0.85em;" onclick="musicControl('play_pause')">⏯️</button>
                        <button class="control-btn" style="padding:6px 10px;font-size:0.85em;" onclick="musicControl('skip')">⏭️</button>
                        <button class="control-btn" style="padding:6px 10px;font-size:0.85em;" onclick="musicControl('volume_up')">🔊</button>
                        <button class="control-btn" style="padding:6px 10px;font-size:0.85em;" onclick="musicControl('volume_down')">🔉</button>
                    </div>
                    <div id="music-status" style="text-align:center;margin-top:6px;font-size:0.75em;"></div>
                </div>
                
                <div class="section" style="padding:10px;">
                    <h2>🔍 Search</h2>
                    <input type="text" id="searchInput" placeholder="Search..." style="width:100%;padding:6px;border-radius:6px;border:none;font-size:0.85em;margin-bottom:5px;">
                    <button class="control-btn" onclick="search()" style="width:100%;padding:6px;font-size:0.85em;">Go</button>
                    <div id="searchResults" style="max-height:80px;overflow-y:auto;margin-top:6px;"></div>
                </div>
            </div>
            
            <div class="section" style="padding:10px;">
                <h2 style="margin-bottom:8px;">💬 Live Feed</h2>
                <div class="recent-feed" style="max-height:210px;overflow-y:auto;">{recent_html}</div>
            </div>
        </div>
        
        <div class="two-column">
            <div class="section">
                <h2>🗣️ Most Talkative</h2>
                <div class="leaderboard">{top_users_html}</div>
            </div>
            
            <div class="section">
                <h2>🤬 Top Swearers</h2>
                <div id="swearStats" class="leaderboard">Loading...</div>
            </div>
        </div>
        ...
        <div class="two-column">
            <div class="section">
                <h2>🎵 Top Songs</h2>
                <div class="leaderboard">{top_songs_html}</div>
            </div>
            
            <div class="section">
                <h2>🕐 Recently Played</h2>
                <div class="leaderboard scrollable">{recent_songs_html}</div>
            </div>
        </div>
    ...
    <script>
        ...
        async function search() {{
            ...
            data.results.forEach(r => {{
                html += `<div class="feed-item">
                    <div class="feed-header">
                        <span class="feed-user">${{r.user}}</span>
        ...
        // Auto-load swear stats on page load
        window.addEventListener('DOMContentLoaded', () => {{
            loadSwearStats();
        }});
        
        async function loadSwearStats() {{
            const resp = await fetch('/word_stats');
            const data = await resp.json();
            
            // Sort by total swears
            const sorted = Object.entries(data)
                .map(([user, stats]) => ({{user, ...stats}}))
                .sort((a, b) => b.total_swears - a.total_swears)
                .slice(0, 10);
            
            let html = '';
            sorted.forEach((item, i) => {{
                const topWords = item.top_swears.map(([word, count]) => `${{word}} (${{count}})`).join(', ');
                html += `
                    <div class="leaderboard-item">
                        <div class="rank">#${{i+1}}</div>
                        <div style="flex:1;">
                            <div class="username">${{item.user}}</div>
                            <div style="font-size:0.85em;opacity:0.7;margin-top:4px;">${{topWords}}</div>
                        </div>
                        <div class="count">${{item.total_swears}} swears</div>
                    </div>
                `;
            }});
            
            document.getElementById('swearStats').innerHTML = html || 'No data';
        }}
    </script>
</body>
</html>"""
...
async def music_control_handler(request):
    ...
    if action == 'play_pause':
        ...
        if guild_id in volume_settings:
            volume_settings[guild_id] = min(1.0, volume_settings[guild_id] + 0.1)
            return web.json_response({'volume': int(volume_settings[guild_id] * 100)})
    
    elif action == 'volume_down':
        if guild_id in volume_settings:
            volume_settings[guild_id] = max(0.0, volume_settings[guild_id] - 0.1)
            return web.json_response({'volume': int(volume_settings[guild_id] * 100)})
...
async def search_handler(request):
    ...
    try:
        ...
        if user:
            c.execute("""SELECT username, transcription, timestamp 
                        FROM transcriptions 
                        WHERE LOWER(transcription) LIKE ? AND username = ?
                        ORDER BY timestamp DESC 
                        LIMIT 50""", (f'%{query}%', user))
        else:
            c.execute("""SELECT username, transcription, timestamp 
                        FROM transcriptions 
                        WHERE LOWER(transcription) LIKE ?
                        ORDER BY timestamp DESC 
                        LIMIT 50""", (f'%{query}%',))
        ...
        return web.json_response({
            'results': [{'user': r[0], 'text': r[1], 'time': r[2]} for r in results]
        })
    except Exception as e:
        return web.json_response({'error': str(e)})

async def word_stats_handler(request):
    """Get top words/swears for users"""
    try:
        conn = get_db_connection()
        c = conn.cursor()
        
        # Get top swear words per user
        c.execute("""SELECT username, transcription FROM transcriptions""")
        all_msgs = c.fetchall()
        conn.close()
        ...
        user_swears = {}
        swear_words = ['fuck', 'fucking', 'fucked', 'fucker', 'fucks',
                      'shit', 'shitty', 'shitting', 'shits',
                      'damn', 'damned', 'damnit',
                      'hell', 'ass', 'asshole', 'asses',
                      'bitch', 'bitches', 'bitching',
                      'bastard', 'bastards', 'crap', 'crappy']
        
        for username, text in all_msgs:
            if username not in user_swears:
                user_swears[username] = Counter()
            
            # Strip all punctuation and get words
            words = re.findall(r'[a-z]+', text.lower())
            
            for word in words:
                if word in swear_words:
                    user_swears[username][word] += 1
        
        # Format for frontend
        stats = {}
        for user, swears in user_swears.items():
            stats[user] = {
                'total_swears': sum(swears.values()),
                'top_swears': swears.most_common(5)
            }
...
async def recent_feed_handler(request):
    """Get recent feed for live updates"""
    try:
        conn = get_db_connection()
        c = conn.cursor()
        ...
        return web.json_response({
            'recent': [{'user': r[0], 'text': r[1], 'time': r[2]} for r in recent]
        })
    except Exception as e:
        return web.json_response({'error': str(e)})
...
async def start_web_server():
    ...
    try:
        ...
        app.router.add_post('/music_control', music_control_handler)
        app.router.add_post('/search', search_handler)
        app.router.add_get('/word_stats', word_stats_handler)
        app.router.add_get('/recent_feed', recent_feed_handler)
